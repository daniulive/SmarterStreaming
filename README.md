...
Beta公测版：超低延迟、可直接推url到rtmp server的windows桌面采集的SDK(为防止未经许可的第三方利用本软件从事恶意盗录等非法活动，未经授权版本，每次只允许实时推送1小时（1小时后，如需继续试用，请关闭应用程序再重新打开），请在法律许可范围内使用本软件。)。
1. 支持帧率、关键帧间隔、码率等设置；
...
3. 点击“获取默认值”，拿到系统推荐的平均码率和最高码率；
4. [音频选择] 采集麦克风：实时获取麦克风数据；
5. [音频选择] 采集扬声器：实时获取扬声器数据，比如，无杂音的采集系统播放的音乐；
6. [音频控制] 推送过程中，可以选择“静音”或取消静音；
7. 设置RTMP推送url；
8. 点击“推送”，如推到daniulive的服务器，系统会自动分配播放url，请按生成的播放地址，用大牛播放器播放；
9. 点击“停止”，关闭推送。
